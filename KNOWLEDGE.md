...
- Archive expansion should inspect one level deeper via archive/program hub pages and raw HTML/script payload URLs, not only direct `<a>` links from the seed page; SPA/Next archive indexes often hide year/event URLs inside JSON blobs instead of anchor tags.
- For JS-only archive/index pages, importer should use a browser-DOM fallback only after cheap HTML/sitemap/payload discovery fails. This fallback reads hydrated `<a>` links from archive/program hubs and reuses the same event-page heuristics, which is necessary for sites like AIJ where `program/2023` and similar URLs appear only after client-side hydration.
